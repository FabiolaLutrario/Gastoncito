...
export default { 
  name: IS_PROD || IS_LOCAL_BUILD ? 'Gastoncito' : (process.env.APP_VARIANT === 'test') ? 'TEST XProv' : 'Dev XProv',
  ...
  ios: {
    supportsTablet: true,
    bundleIdentifier: IS_PROD || IS_LOCAL_BUILD ? 'gastoncito.prod' : 'gastoncito.dev',
  },
  android: {
    permissions: [
      'CAMERA',
    ...
    package: IS_PROD || IS_LOCAL_BUILD ? 'gastoncito.prod' : (process.env.APP_VARIANT === 'test') ? 'gastoncito.test' : 'gastoncito.dev',
  },
  web: {
